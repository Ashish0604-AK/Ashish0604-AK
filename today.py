...
def daily_readme(birthday):
    """
    Returns the length of time since I was born in short format
    e.g. '22y 4m 19d'
    """
    diff = relativedelta.relativedelta(datetime.datetime.today(), birthday)
    
    cake = ' 🎂' if (diff.months == 0 and diff.days == 0) else ''
    return f"{diff.years}y {diff.months}m {diff.days}d{cake}"
...
def justify_format(root, element_id, new_text, length=21):
    """
...
if __name__ == '__main__':
    """
    Ashish0604-AK (Ash_06), 2023-2027
    """
    ...
    age_data, age_time = perf_counter(daily_readme, datetime.datetime(2004 , 4 , 6))  # Replace YYYY, MM, DD
